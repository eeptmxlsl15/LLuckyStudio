...
public class InfiniteRankLoader : MonoBehaviour
{
	[SerializeField]
	...
	[SerializeField]
	private InfiniteRankData myRankData;

	private List<InfiniteRankData> rankDataList;

	private void Awake()
	{
		rankDataList = new List<InfiniteRankData>();

		for(int i = 0; i < Constants.MAX_RANK_LIST; ++i)
		{
			GameObject clone = Instantiate(rankDataPrefab, rankDataParent);
			rankDataList.Add(clone.GetComponent<InfiniteRankData>());
		}
	}
	...
	{
		Backend.URank.User.GetRankList(Constants.INFINITE_RANK_UUID, Constants.MAX_RANK_LIST, callback =>
		{
	...
	private void SetRankData(InfiniteRankData rankData, int rank, string nickname,int score)
	{
		rankData.Rank = rank;
